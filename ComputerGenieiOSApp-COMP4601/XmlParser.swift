...
class XmlParser: NSObject {
    ...
    class func parseUser(xml: AEXMLDocument) -> User {
        
        let token = xml.root["authToken"].value!
        let id = xml.root["id"].value!
        let password = xml.root["password"].value!
        ...
        var result = User(token: token, id: id, email: email, password: password, name: firstname + " " + lastname, birthdate: birthday, gender: gender, lastLogin: lastLogin, productHistory: productHistory)
